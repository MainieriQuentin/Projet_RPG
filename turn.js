class Turn extends Game {
    ...
    switch (playerResponse) {
      ...
      case "4":
        if (
          ...
          player.constructor.name == "Hunter" ||
      ...
      default:
        alert("Désolé, je n'ai pas comprit. Réessayez s'il vous plait!");
        this.playerActionsMenu(player);
    }
  ...
  chooseTarget(player) {
    ...
    for (let i in targets) {
      question += `\nAppuyez [${i}] pour attaquer ${targets[i].name}`;
    }
  ...
  deactivateFigthersUlt(players) {
    players
      ...
      .map((player) => player.resetUlt());
  }

  deactivateAssassinsUlt(players) {
    players
      .filter((player) => player.constructor.name == "Assassin")
      .map((player) => player.resetUlt());
  }
  ...
  performAssassination(players) {
    players
      .filter((player) => player.constructor.name == "Assassin")
      .filter((player) => player.target != 0)
      .map((player) => player.assassination());
  }
}
